...
def StaticSpecs():
    ...
    OS = getOsImage()
    return (PrivateIP + ':_:' + MAC + ':_:' + OS +':_:' + str(RAM_total) + ':_:' + str(DISK_total))


def CurrentSpecs():
    threading.Timer(20.0, CurrentSpecs).start()
    ...
    PrivateIP = getPrivateIP()
    return (CPU_temp + ':_:' + CPU_usage + ':_:' + str(RAM_used) + ':_:' + str(DISK_perc))
